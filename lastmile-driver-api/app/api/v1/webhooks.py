"""
Recebe o webhook do gateway evolution-go e o webhook de cobrança do Asaas.
Ambos sempre respondem 200 (mesmo para eventos ignorados/duplicados/tenant
desconhecido) — são webhooks, não API de negócio: devolver erro faria o
emissor reenviar o mesmo evento, piorando exatamente o cenário de
duplicação que a idempotência já cobre.
"""
import logging
...
async def receive_asaas_webhook(
    ...
    princípio do webhook do evolution-go: sempre 200, pra não entrar em loop
    de reenvio do lado do Asaas em caso de evento não reconhecido."""
