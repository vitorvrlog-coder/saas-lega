...
class FailureClassificationOutput(AIOutputBase):
    ...
    order_number: número da nota fiscal/pedido, só preenchido se o motorista
    citar (ex: "nota 12345"). Motoristas de distribuidora (produto de nota
    fiscal via app, sem planilha de rota) referenciam a entrega pelo número
    da NF em vez de parada — usado por
    app.services.invoice_service.find_confirmed_invoice_by_order_number como
    alternativa a stop_number. Mesma disciplina: nunca inventado."""
...
class RoutePrescreenCategory(str, enum.Enum):
    CONFIRMED = "confirmed"
...
class RoutePrescreenReplyOutput(AIOutputBase):
    """Classificação da resposta do cliente à pré-triagem de rota (Route
    Capture) — ver app.services.route_prescreen_service."""

    category: RoutePrescreenCategory
    # Só preenchido quando category == ADDRESS_WRONG e o cliente informou
    # o endereço completo novo.
    new_address_text: str | None = None
